...
class Client:
    ...
    def process(self, msg=b"", newMSG=""):
        ...
        if self.msg_cnt == 2:
            ...
            self.propria_private_key = private_key
        ...
        if self.flag == 1 and msg != b'':
            ...
            if msg.startswith(b"["):
                ...
                first_four_fields = mensagens[:4]
                field1 = first_four_fields[0]
                field2 = first_four_fields[1]
                field3 = first_four_fields[2]
                field4 = first_four_fields[3]
                print("Msg number = ", field1.decode())
                print("Sender = ", field2.decode())
                print("Tempo = ", field3.decode())

                sub = self.propria_private_key.decrypt(
                    field4,
                    padding.OAEP(
                ...
                print("Subject = ", sub.decode())


            if msg.startswith(b"MSG"):
                
                mensagens = msg.split(b" _separar_ ")
                first_four_fields = mensagens[:3]
                field1 = first_four_fields[0]
                field2 = first_four_fields[1]
                field3 = first_four_fields[2]
                print("Sender =", field1.decode())
                sub = self.propria_private_key.decrypt(
                    field2,
                    padding.OAEP(
                        mgf=padding.MGF1(algorithm=hashes.SHA256()),
                ...
                print("Subject =", sub.decode())
                content = self.propria_private_key.decrypt(
                    field3,
                    padding.OAEP(
                        mgf=padding.MGF1(algorithm=hashes.SHA256()),
                        algorithm=hashes.SHA256(),
                        ...
                        while True:
                            if new_msg.startswith(b"send"):
                                ...
                                if len(arguments) == 2:
                                    ...
                                    path = "public_keys/MY_" + user_UID.decode() + ".public_key.pem"
                                    with open(path, "rb") as f:
                                        pem_data = f.read()


                                    # Carregar a chave pública a partir do PEM
                                    reciver_public_key = serialization.load_pem_public_key(pem_data)

                                    subject_cipher =  reciver_public_key.encrypt(send_Subjet,padding.OAEP( mgf=padding.MGF1(algorithm=hashes.SHA256()),algorithm=hashes.SHA256(),label=None))
                                    text_cipher = reciver_public_key.encrypt(text,padding.OAEP( mgf=padding.MGF1(algorithm=hashes.SHA256()),algorithm=hashes.SHA256(),label=None))
                                    new_msg = b"send" + b" separar " + user_UID + b" separar " + subject_cipher + b" separar " + text_cipher
