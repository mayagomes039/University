<script setup>
    ...
    const fetchProject = async () => {
        ...
        try {
            const response = await axios.get(`/api/projects/project/${projectId.value}`, {withCredentials: true});
   ...
   const setupSocketHandler = (socket) => {
    ...
    if (!socket) {
        console.warn("Socket is null or undefined. Exiting handler setup.");
        return;
    }
    ...
    socket.onmessage = (event) => {
        ...
        if (!data2 || typeof data2 !== "object" || !data2.data) {
            console.warn("Received invalid or incomplete data:", data2);
            return; // Exit if data2 is not an object or data field is missing
        }
        ...
        // Ensure data.projectId and projectId.value exist and match
        if (data?.projectId === projectId?.value) {
            if (data.status === "failed") {
                console.log("Processing failed:", data.error);
                isLoading.value = false;
                fileSizeError.value = true;
                fileName.value = `Erro ao processar ferramenta: ${data.error || "Unknown error"}`;
                fetchProject();
            } else if (data.status === "finished") {
                console.log("Processing finished, entering handler");
                handleFinishedProcessing(data);
            } else {
                console.warn("Unexpected status:", data.status);
            }
        } else {
            console.warn("Project ID mismatch or missing:", {
                received: data?.projectId,
                    ...
                    <div class="image-list-section">
                        ...
                        <ul>
                            <li v-for="(image, index) in project.project.images" :key="index" class="image-item">
                                ...
                                <img :src="addCacheFix(`/s3/` + image.uri)" @click="selectImage(image.uri, image.id)" alt="Image" />
                            </li>
                        </ul>
                    </div>
                </div>
