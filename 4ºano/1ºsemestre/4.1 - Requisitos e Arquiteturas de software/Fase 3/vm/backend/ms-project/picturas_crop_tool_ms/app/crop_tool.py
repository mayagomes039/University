...
from .crop_result_message import CropResultOutput
...
class CropTool(Tool):
    ...
    def apply(self, parameters: CropParameters):
        """
        Crop the input image and save the output.

        Args:
            parameters (CropParameters): Parameters containing input and output paths.

        Returns:
            CropResultOutput: Contains the type and URI of the processed image.
        ...
        try:
            ...
            with Image.open(temp_input) as input_image:
                # Crop the image using individual coordinates
                cropped_image = input_image.crop((parameters.left, parameters.upper, parameters.right, parameters.lower))

                # Save the cropped image to the temporary output file
                cropped_image.save(temp_output)
            ...
            result = CropResultOutput(type="image", imageURI=f"{s3BucketName}/{parameters.outputImageURI}")
            return result
