...
const editProfile = async () => {
  try {
    ...
    const response = await axios.post(
      `http://localhost:3000/api/users/user/edit/${id.value}`, // Usando o método POST para editar o perfil
      {
        name: userName.value,
        email: email.value,
    ...
    if (response.status === 200) {
      ...
      try {
        ...
        if (currentPlan != 'free') {
          const response3 = await axios.post('http://localhost:3000/api/subscriptions/subscription/cancel/', {
          userId: getId(),
          }, {
            withCredentials: true,
          });
        ...
        if(window.localStorage.getItem('newPlan') != 'free') {
                ...
                const response2 = await axios.post('http://localhost:3000/api/subscriptions/subscription/', {
                    userId: getId(),
